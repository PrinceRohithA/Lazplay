...
const storageDb = {
  ...
  getTokens: () => {
    ...
    return { token: token == null ? void 0 : token.value, refreshToken: refreshToken == null ? void 0 : refreshToken.value };
  },
  getGame: (id) => {
...
class DownloadManager {
  ...
  async startInstall(gameId) {
    const installPath = require$$1$1.join(
      require$$1.app.getPath("userData"),
      "installed-games",
      gameId
    );
    if (!fs$j.existsSync(installPath)) {
      fs$j.mkdirSync(installPath, { recursive: true });
    }
    storageDb.setGameStatus(gameId, "downloading", {
      title: `Game ${gameId}`,
      // In real app, fetch metadata from API
      installPath
    });
    const cdnUrl = "https://example.com/mock-game.zip";
    this.downloadFile(gameId, cdnUrl, require$$1$1.join(installPath, "game.zip"));
  }
  downloadFile(gameId, url, destination) {
    let downloadedBytes = 0;
    let totalBytes = 1e8;
    const stream = fs$j.createWriteStream(destination, { flags: "a" });
  ...
  finishDownload(gameId, filePath) {
    ...
    fs$j.writeFileSync(
      require$$1$1.join(extractPath, "executable.exe"),
      "mock exe content"
    );
  ...
  async pauseDownload(gameId) {
    ...
    if (active) {
      ...
      active.stream.close();
      this.activeDownloads.delete(gameId);
      storageDb.setGameStatus(gameId, "paused");
  ...
  async uninstall(gameId) {
    ...
    if (game && game.installPath) {
      if (fs$j.existsSync(game.installPath)) {
        fs$j.rmSync(game.installPath, { recursive: true, force: true });
      }
      storageDb.removeGame(gameId);
...
class ProcessManager {
  constructor() {
    __publicField(this, "runningGames", /* @__PURE__ */ new Map());
  }
  async launchGame(gameId) {
    ...
    const child = require$$0.spawn(exePath, [], {
      ...
      detached: true,
      stdio: "ignore"
      // For production, we might redirect stdio to a log file
    });
    child.unref();
...
function setupIpcHandlers(mainWindow2, storeView2) {
  require$$1.ipcMain.handle("sync-session", async (event, { token, refreshToken }) => {
    ...
    storageDb.setTokens(token, refreshToken);
  ...
  require$$1.ipcMain.handle("launch-game", async (event, gameId) => {
    log.info(`Launch requested for game: ${gameId}`);
    try {
      await processManager.launchGame(gameId);
      return { success: true };
    } catch (error2) {
      log.error(`Launch failed for ${gameId}:`, error2);
      return { success: false, error: error2.message };
    }
  });
  require$$1.ipcMain.handle("uninstall-game", async (event, gameId) => {
    log.info(`Uninstall requested for game: ${gameId}`);
    try {
      await processManager.stopGame(gameId);
      await downloadManager.uninstall(gameId);
      return { success: true };
    } catch (error2) {
      log.error(`Uninstall failed for ${gameId}:`, error2);
      return { success: false, error: error2.message };
    }
  });
  require$$1.ipcMain.handle("pause-download", async (event, gameId) => {
    return downloadManager.pauseDownload(gameId);
  });
  require$$1.ipcMain.handle("resume-download", async (event, gameId) => {
  ...
  require$$1.ipcMain.handle("get-installed-games", () => {
    return storageDb.getInstalledGames();
  });
  require$$1.ipcMain.handle("get-running-games", () => {
    return processManager.getRunningGames();
  });
  require$$1.ipcMain.handle("get-download-progress", (event, gameId) => {
    return downloadManager.getProgress(gameId);
  });
  require$$1.ipcMain.handle("get-disk-usage", async () => {
    return {
      free: 1e11,
      total: 5e11
    };
  });
  require$$1.ipcMain.handle("set-store-visibility", (event, visible) => {
    if (storeView2 && mainWindow2) {
      if (visible) {
        mainWindow2.contentView.addChildView(storeView2);
      } else {
...
function initAutoUpdater(mainWindow2) {
  ...
  main$1.autoUpdater.logger.transports.file.level = "info";
  ...
  main$1.autoUpdater.on("download-progress", (progressObj) => {
    ...
    mainWindow2.webContents.send("update-progress", progressObj);
  });
...
require$$1.app.whenReady().then(() => {
  if (process.defaultApp) {
    ...
    require$$1.app.setAsDefaultProtocolClient("lazplay");
  }
  createWindow();
