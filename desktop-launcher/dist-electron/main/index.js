...
class DownloadManager {
  ...
  async startInstall(gameId) {
    ...
    if (!fs$1.existsSync(installPath)) {
      fs$1.mkdirSync(installPath, { recursive: true });
    }
  ...
  downloadFile(gameId, url, destination) {
    ...
    let totalBytes = 1e8;
    const stream = fs$1.createWriteStream(destination, { flags: "a" });
  ...
  async uninstall(gameId) {
    ...
    if (game && game.installPath) {
      if (fs$1.existsSync(game.installPath)) {
        fs$1.rmSync(game.installPath, { recursive: true, force: true });
      }
