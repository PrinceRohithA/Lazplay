...
export default function PlayerDiscoveryHub() {
  ...
  return (
    <div className={`flex flex-col gap-6 md:gap-margin p-4 md:p-margin ${isStandard ? '' : 'grid-glow-bg'}`}>
      <section className="w-full relative border-2 border-outline-variant bg-surface-container-low retro-border min-h-[300px] md:h-[400px] flex flex-col mb-4 md:mb-gutter">
        <div className="bg-surface-container-highest border-b-2 border-outline-variant px-4 py-1 flex justify-between items-center">
          <span className="font-label-mono text-[10px] md:text-label-mono text-on-primary-container">
            {isStandard ? '' : '> '}{t('FEATURED_PROTOCOL.EXE')}
          </span>
          <div className="flex gap-2">
            {(featured.length ? featured : [0,1,2]).map((_, i) => (
              <button key={i} onClick={() => setHeroIdx(i)} className={`w-2 h-2 md:w-3 md:h-3 border transition-colors ${i === heroIdx ? 'bg-primary border-primary' : 'bg-surface border-on-surface'}`} />
            ))}
          </div>
        </div>
        <div className="flex-1 relative w-full h-full min-h-[250px] md:min-h-0">
          {loading ? (
            <div className="w-full h-full flex items-center justify-center">
              <span className="font-label-mono text-on-primary-container animate-pulse">{t('LOADING_FEATURED_PROTOCOL...')}</span>
            </div>
          ) : hero ? (
            <div className="absolute inset-0 flex flex-col">
              <img alt={hero.title} className={`absolute inset-0 w-full h-full object-cover transition-all duration-500 ${isStandard ? '' : 'opacity-80 mix-blend-luminosity grayscale hover:grayscale-0'}`} src={hero.heroBannerUrl || hero.heroImageUrl || 'https://lh3.googleusercontent.com/aida-public/AB6AXuCDB5l8_2ZRmLC1n16I24FMMADDCZxwS6JscWsm2D8MvdCzka-NjfuHvd_KjhYScjv_b19ngm1WWqsgwoUxV4ygzPoKB-a8BB4QszlDn6v285IQFQqlgCmVmBrCvYMd4enHnsKJWmOBHBQTNNstZKFFrQLEjCFaeTf-5Hw_KaShDa_2doPNvpvBWjSo47gwaC_Yw5euMOvcnouvcTudO3NwNYXoFnIs8lRnQl2AVpI1AFXFovc9u5MvAocO1uqKXKlaGsiOXoQKVaOi'} />
              <div className="absolute inset-0 bg-gradient-to-t from-background via-background/20 to-transparent flex flex-col md:flex-row justify-end md:justify-between items-start md:items-end p-4 md:p-8 gap-4">
                <div className="max-w-2xl">
                  <h1 className="font-headline-lg md:font-headline-xl text-headline-md md:text-headline-xl text-on-primary-container font-bold glow-text-primary mb-2 uppercase tracking-tight truncate w-full">
                    {isStandard ? hero.title : hero.title?.replace(/\s/g,'_')}
                  </h1>
                  <p className="hidden sm:block font-body-md md:font-body-lg text-on-surface-variant bg-surface-container-highest/80 p-2 border border-outline-variant">
                    {hero.tagline || (isStandard ? 'High-speed synthwave racing game. Engage hyper-drive.' : 'HIGH-SPEED SYNTHWAVE RACING PROTOCOL. ENGAGE HYPER-DRIVE.')}
                  </p>
                </div>
                <Link 
                  to={`/game?id=${hero.id}`} 
                ...
                >
                  {hero.isOwned ? t('EXECUTE_PLAY') : hero.priceType === 'FREE' ? t('CLAIM_FREE') : t('VIEW_OFFER')}
                </Link>
              </div>
            </div>
          ) : (
            <div className="w-full h-full flex items-center justify-center">
              <span className="font-label-mono text-on-surface-variant">{t('NO_FEATURED_GAMES_FOUND')}</span>
            </div>
          )}
        </div>
      </section>
        ...
        <section className="space-y-4">
          <div className="flex justify-between items-end border-b-2 border-outline-variant pb-2">
            <h2 className="font-headline-md text-headline-sm md:text-headline-md text-on-surface uppercase tracking-wide flex items-center gap-2">
              {!isStandard && <span className="text-on-primary-container glow-text-primary">&gt;</span>} {t('TRENDING_MATRIX')}
            </h2>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 gap-4">
            {featured.slice(5, 12).map((game, i) => (
              <Link 
                key={game.id} 
                to={`/game?id=${game.id}`}
                className="group relative aspect-[3/4] bg-surface-container border border-outline-variant hover:border-primary transition-all overflow-hidden pixel-border"
              >
                <img 
                  ...
                  className={`w-full h-full object-cover transition-all ${isStandard ? '' : 'grayscale group-hover:grayscale-0'}`}
                  alt={game.title}
                />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-margin">
        <div className="lg:col-span-9 space-y-4">
          <div className="flex justify-between items-end border-b-2 border-outline-variant pb-2">
            <h2 className="font-headline-md text-headline-sm md:text-headline-md text-on-surface uppercase tracking-wide flex items-center gap-2">
              {!isStandard && <span className="text-on-primary-container glow-text-primary">&gt;</span>} {t('MEMORY_BANKS')}
            </h2>
            <Link to="/library" className="font-label-mono text-[10px] md:text-label-mono text-on-surface-variant hover:text-on-primary-container underline decoration-dotted">{t('VIEW_ALL')}</Link>
          </div>
          {loading ? (
              ...
              {recentGames.map(item => (
                <div key={item.gameId} className="bg-surface-container border-2 border-outline-variant retro-border retro-border-hover group transition-all flex flex-col h-full">
                  <div className="h-32 border-b-2 border-outline-variant relative overflow-hidden bg-surface-container-lowest">
                    <img alt={item.title} className={`w-full h-full object-cover transition-opacity duration-300 ${isStandard ? '' : 'opacity-60 group-hover:opacity-100 grayscale group-hover:grayscale-0'}`} src={item.heroBannerUrl || item.heroImageUrl || item.coverUrl || 'https://lh3.googleusercontent.com/aida-public/AB6AXuC54pm-DlFwdN1M9aKkTmci1Zz0d7megKplAw_Mc7TTOX7uLShonn-bBLCc7cXpCoaJEvbpeFbBK5hIhZIS2jwNMGNDpJjWbSqi2Fr3cNsnrU1Jy2gnBW2BwF7ogB2l8jrpkOJynVla_CZmJsnzn6FvKxT3vjKF21JJYB7_pm9W8NAIEct109nk6rV1xJ_FCd0xRJD1IbvQaP1GVxe8QCxci3u4RBQ6gSKWdCufRjGqG4WmLbZnPYsXoZg7mIms2ZbZOqH65wIHmGoM'} />
                    <div className="absolute top-2 right-2 bg-primary text-on-primary-container font-label-mono text-[10px] px-2 py-0.5 border border-on-primary">{t(item.installedStatus || 'READY')}</div>
                  </div>
                  <div className="p-4 flex flex-col flex-1">
                    <h3 className="font-headline-md text-[18px] md:text-[20px] text-on-surface mb-1 group-hover:text-primary transition-colors uppercase">
                      {isStandard ? item.title : (item.title||'').replace(/\s/g,'_')}
                    </h3>
                    <p className="font-label-mono text-[10px] text-on-surface-variant mb-4">
                      {item.lastPlayedAt ? `${t('LAST_SYNC')}: ${new Date(item.lastPlayedAt).toLocaleDateString()}` : t('NEVER_PLAYED')}
                    </p>
                    <div className="mt-auto flex justify-between items-center">
                      <div className="flex gap-1 h-2 w-16 bg-surface-container-highest border border-outline">
                        <div className="bg-primary" style={{ width: item.updateAvailable ? '50%' : '100%' }} />
                      </div>
                      <Link to={`/game?id=${item.gameId}`} className="text-on-primary-container hover:text-primary-fixed bg-surface border border-outline-variant hover:border-primary px-3 py-1 font-label-mono text-[10px] cyber-btn">{t('RESUME')}</Link>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="text-center py-12 font-label-mono text-on-surface-variant border-2 border-dashed border-outline-variant p-4">
              <p>{t('NO_GAMES_IN_LIBRARY')}</p>
              <Link to="/games" className="text-primary underline mt-2 block">{t('BROWSE_CATALOG')}</Link>
            </div>
          )}
        </div>
        ...
        <aside className="lg:col-span-3 space-y-4">
          <div className="border-b-2 border-outline-variant pb-2">
            <h2 className="font-headline-md text-headline-sm md:text-[20px] text-on-surface uppercase tracking-wide flex items-center gap-2">
              {!isStandard && <span className="text-primary glow-text-primary">&gt;</span>} {t('NETWORK_STATUS')}
            </h2>
          </div>
          <div className="bg-surface border-2 border-outline-variant p-4 space-y-4 retro-border">
            <div className="bg-surface-container-highest border-b-2 border-outline-variant px-2 py-1 mb-2">
              <span className="font-label-mono text-[10px] text-on-surface-variant uppercase">{t('ACTIVE_CONNECTIONS.LOG')}</span>
            </div>
            <div className="space-y-2">
              {[
                { label: 'FEATURED_GAMES', value: featured.length },
                { label: 'LIBRARY_SIZE', value: recentGames.length },
                { label: 'BACKEND', value: loading ? t('CONNECTING...') : t('ONLINE') },
              ].map(({ label, value }) => (
                <div key={label} className="flex justify-between font-label-mono text-[10px]">
                  <span className="text-on-surface-variant uppercase">{t(label)}</span>
