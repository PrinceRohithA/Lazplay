    ...
    <div className="flex flex-col gap-6 md:gap-margin p-4 md:p-margin">
      <section className="w-full relative border-2 border-outline-variant bg-surface-container-low retro-border min-h-[300px] md:h-[400px] flex flex-col mb-4 md:mb-gutter">
        <div className="bg-surface-container-highest border-b-2 border-outline-variant px-4 py-1 flex justify-between items-center">
          <span className="font-label-mono text-[10px] md:text-label-mono text-primary-container">&gt; FEATURED_PROTOCOL.EXE</span>
            ...
            <div className="absolute inset-0 flex flex-col">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-background via-background/20 to-transparent flex flex-col md:flex-row justify-end md:justify-between items-start md:items-end p-4 md:p-8 gap-4">
                <div className="max-w-2xl">
                  <h1 className="font-headline-lg md:font-headline-xl text-headline-md md:text-headline-xl text-primary-container font-bold drop-shadow-[0_0_8px_rgba(57,255,20,0.8)] mb-2 uppercase tracking-tight truncate w-full">{hero.title?.replace(/\s/g,'_')}</h1>
                ...
                <Link 
                  to={`/game?id=${hero.id}`} 
                  className={`w-full md:w-auto text-center font-label-mono text-label-mono px-6 py-3 border-2 transition-colors shadow-[4px_4px_0_0_#107100] active:translate-y-1 active:translate-x-1 active:shadow-[0_0_0_0_#107100] ${
                    hero.isOwned 
        ...
        <section className="space-y-4">
          <div className="flex justify-between items-end border-b-2 border-outline-variant pb-2">
            <h2 className="font-headline-md text-headline-sm md:text-headline-md text-on-surface uppercase tracking-wide flex items-center gap-2">
              <span className="text-primary-container">&gt;</span> TRENDING_MATRIX
            </h2>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-margin">
        <div className="lg:col-span-9 space-y-4">
          <div className="flex justify-between items-end border-b-2 border-outline-variant pb-2">
            <h2 className="font-headline-md text-headline-sm md:text-headline-md text-on-surface uppercase tracking-wide flex items-center gap-2">
              <span className="text-primary-container">&gt;</span> MEMORY_BANKS
            </h2>
            <Link to="/library" className="font-label-mono text-[10px] md:text-label-mono text-on-surface-variant hover:text-primary-container underline decoration-dotted">VIEW_ALL</Link>
          </div>
          {loading ? (
              ...
              {recentGames.map(item => (
                <div key={item.gameId} className="bg-surface-container border-2 border-outline-variant retro-border retro-border-hover group transition-all flex flex-col h-full">
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    <h3 className="font-headline-md text-[18px] md:text-[20px] text-on-surface mb-1 group-hover:text-primary-container transition-colors uppercase">{(item.title||'').replace(/\s/g,'_')}</h3>
